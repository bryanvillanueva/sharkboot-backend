  ...
  router.get('/facebook/callback', async (req, res) => {
    try {
      ...
      try {
        // Decodificar state
        const decodedState = decodeURIComponent(state);
        console.log('🔍 State decodificado tipo:', typeof decodedState, 'inicio:', decodedState.substring(0, 20));
        
        // ✅ VERIFICAR SI ES BASE64 O JSON DIRECTO PRIMERO
        let jsonString = decodedState;
        let isBase64Encoded = false;
        
        // Verificar si parece ser base64 (caracteres válidos y longitud apropiada)
        if (/^[A-Za-z0-9+/=]+$/.test(decodedState) && decodedState.length % 4 === 0) {
          try {
            jsonString = Buffer.from(decodedState, 'base64').toString('utf8');
            isBase64Encoded = true;
            console.log('🔓 State decodificado desde base64:', jsonString.substring(0, 100));
          } catch (base64Error) {
            console.log('⚠️ No se pudo decodificar como base64, usando string original');
            jsonString = decodedState;
          }
        }
        
        // ✅ VERIFICAR SI ES UN JWT REAL (tiene 3 partes separadas por puntos)
        if (!isBase64Encoded && jsonString.includes('.') && jsonString.split('.').length === 3) {
          console.log('🔑 Detectado JWT real, intentando decodificar...');
          try {
            const jwtParts = jsonString.split('.');
            const payload = JSON.parse(atob(jwtParts[1]));
            console.log('📋 JWT payload:', payload);
            
            // ✅ CREAR stateData desde JWT
            stateData = {
              timestamp: Date.now(),
          ...
          console.log('📄 Parseando como JSON normal:', jsonString.substring(0, 100));
          stateData = JSON.parse(jsonString);
          console.log('✅ State parseado como JSON:', stateData);
          
          // ✅ SANITIZAR datos de JSON
          stateData = {
            ...
            linkToUserId: stateData.linkToUserId || null
          };
          
          console.log('✅ State sanitizado:', stateData);
        }
      } catch (parseError) {
      ...
      // ✅ VALIDACIONES ESPECÍFICAS POR TIPO DE FLUJO
      if (stateData.source === 'whatsapp_setup') {
        if (!stateData.userId || !stateData.clientId) {
      ...
      if (stateData.source === 'link_facebook') {
        // Para linking, necesitamos linkToUserId
        if (!stateData.linkToUserId) {
          console.error('❌ Vinculación de Facebook requiere linkToUserId válido');
          const frontendUrl = stateData.frontend_url || 'http://localhost:5173';
          return res.redirect(`${frontendUrl}/accounts?error=invalid_session&error_description=${encodeURIComponent('Sesión inválida para vinculación')}`);
        }
      }
      ...
      // ✅ DETECTAR TIPO DE FLUJO - CORREGIDO
      const isLinking = !!(stateData.linkToUserId || (stateData.source === 'link_facebook'));
      const isWhatsAppSetup = stateData.source === 'whatsapp_setup';
      
      console.log('🔍 Detectando flujo:', {
        isLinking,
        isWhatsAppSetup,
      ...
      if (isWhatsAppSetup) {
        ...
        try {
          // ✅ VALIDAR parámetros antes de la consulta SQL
          if (!stateData.userId) {
            throw new Error('userId no encontrado en el state para setup de WhatsApp');
          }
          
          console.log('🔍 Buscando usuario con ID:', stateData.userId);
          ...
          if (!existingUser) {
            console.error('❌ Usuario no encontrado con ID:', stateData.userId);
      ...
      if (isLinking) {
        ...
        const userIdToLink = stateData.linkToUserId;
        console.log('🔗 Vinculando Facebook a cuenta existente:', userIdToLink);
        
        // ✅ VALIDAR que tenemos un userId para vincular
        if (!userIdToLink) {
          throw new Error('No se encontró userId para vincular Facebook');
        }
        ...
        try {
          ...
          if (existingProvider) {
            ...
            'UPDATE user_providers SET provider_id = ?, access_token_enc = ? WHERE user_id = ? AND provider = "FACEBOOK"',
              [facebook_id, access_token, userIdToLink]
            );
            ...
            await db.execute(
              'INSERT INTO user_providers (id, user_id, provider, provider_id, access_token_enc, created_at) VALUES (?, ?, "FACEBOOK", ?, ?, NOW())',
              [newProviderId, userIdToLink, facebook_id, access_token]
            );
          ...
          const frontendUrl = stateData.frontend_url || 'http://localhost:5173';
          const redirectUrl = new URL(frontendUrl + '/accounts');
          redirectUrl.searchParams.set('facebook_linked', 'true');
          redirectUrl.searchParams.set('auth_token', token);
          
          console.log('🔄 Facebook vinculado, redirigiendo a accounts');
          return res.redirect(redirectUrl.toString());
          
        } catch (linkError) {
          console.error('❌ Error vinculando Facebook:', linkError.message);
          const frontendUrl = stateData.frontend_url || 'http://localhost:5173';
          return res.redirect(`${frontendUrl}/accounts?error=link_failed`);
        }
